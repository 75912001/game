...
// LoadEnemyPlaneFrames 加载-敌机-动画帧
func LoadEnemyPlaneFrames(id uint32, level uint32, frameCount uint32) (frames []*ebitenv2.Image, frameWidth float64, frameHeight float64, err error) {
	// 构建图片路径
	...
	img, _, err := ebitenv2ebitenutil.NewImageFromFile(planePath)
	if err != nil {
		log.Printf("加载敌机图片失败:%v %v", planePath, err)
		return nil, 0, 0, err
	}
