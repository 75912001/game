...
type TiledMap struct {
	...
	Orientation  string          // 地图方向: "staggered"(等距交错), "orthogonal"(正交), "isometric"(等距)
	StaggerAxis  string          // 交错轴: "x" 或 "y"
	StaggerIndex string          // 交错索引: "odd" 或 "even"
...
type TiledLayer struct {
	ID      int            // 图层ID
	Name    string         // 图层名称
	Type    string         // 图层类型: "tilelayer", "objectgroup"
	Visible bool           // 是否可见
	Opacity float64        // 透明度
	X       int            // 图层X偏移
	Y       int            // 图层Y偏移
	Width   int            // 图层宽度
	Height  int            // 图层高度
	Chunks  []*TiledChunk  // chunk 列表（用于 infinite 地图）
	Data    []int          // tile 数据（用于有限地图）
	Objects []*TiledObject // 对象列表（用于 objectgroup）
}

// TiledChunk Tiled 数据块（用于 infinite 地图）
type TiledChunk struct {
	X      int   // chunk 起始 X（tile 坐标）
	Y      int   // chunk 起始 Y（tile 坐标）
	Width  int   // chunk 宽度（tile 数）
	Height int   // chunk 高度（tile 数）
	Data   []int // tile ID 数组
}
...
func (m *TiledMap) GetPixelHeight() int {
	switch m.Orientation {
	case "staggered":
		// 等距交错地图：高度 = (行数 + 1) * (tileHeight / 2)
		return (m.Height + 1) * (m.TileHeight / 2)
	case "isometric":
