...
func (p *User) Login(account string, password string) error {
	// 初始化用户数据 - 模拟数据
	{
		...
		roleRecord.AssetIDRecordMap[uint32(proto.AssetIDRecord_AssetIDRecord_X)] = 100                                                        // 当前位置x
		roleRecord.AssetIDRecordMap[uint32(proto.AssetIDRecord_AssetIDRecord_Y)] = 100                                                        // 当前位置y
